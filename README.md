...
- 认输、悔棋、重开、双 AI 暂停与继续；AI 对弈模式可选择任一 AI 作为认输方
- 四种棋均实时显示双方胜率百分比；进行中使用本地局面启发式估算，终局按实际结果显示
...
胜率计算完全在浏览器本地完成，不会增加模型请求。进行中的棋局根据五子棋连珠威胁、象棋和国际象棋子力与位置、围棋盘面与提子等信息做启发式估算，用于观察局面趋势；它不等同于专业棋力引擎的精确预测。认输、胜负或和棋确定后，百分比会切换为实际终局结果。
...
提示词会把合法 ID 与棋谱记法按 `ID | 棋谱记法` 分列。应用优先精确匹配当前局面的合法 ID；若兼容模型误把显示记法一并复制（例如 `e2e4[e4]`），会去除附注后再次校验。模型只返回 SAN 或象棋记法时，必须能唯一对应到一条当前合法着法才会被接受。
